...
class OptimizedUnivariateEncoding:
    ...
    treatment_groups: dict mapping str to dict mapping Part to int
        The keys are the variable names. The values are themselves dictionaries, which keys are groups
        or intervals and which values are numbers.
    """

    def __init__(self):
        self.model: dict[str, Partition] = {}
    ...
    def target_treatment_pairs(self):
        ...
        """
        return [join_jt(target, treatment) for target in self.target_modalities for treatment in self.treatment_modalities]
    ...
    def fit(self, data, treatment_col, target_col, maxparts = None):
        ...
        for variable in docroot['detailed statistics']:
            ...
            if vardim['partitionType'] == 'Value groups':
                default_part_index = vardim['defaultGroupIndex']
                partition = []
                for part_index, part in enumerate(vardim['partition']):
                    value_group = khiops.core.PartValueGroup(part)
                    if part_index == default_part_index:
                        value_group.is_default_part = True
                    partition.append(value_group)
                model[varname] = partition
            elif vardim['partitionType'] == 'Intervals':
                partition = list(map(khiops.core.PartInterval, vardim['partition']))
                first_non_missing_interval_index = next(interval_index for interval_index, interval in enumerate(partition) if not interval.is_missing)
                partition[first_non_missing_interval_index].is_left_open = True
                partition[-1].is_right_open = True
                model[varname] = partition
        ...
        self.model = model
        self.levels = levels
        self.variable_cols = data.apply(lambda column: column.apply(str) if variable_types[column.name] == "Categorical" else column)
        self.treatment_col = treatment_col.apply(str)
        self.target_col = target_col.apply(str)
    ...
    def transform(self, data):
        ...
        pd.DataFrame
            Pandas Dataframe that contains encoded data.
        """
        data = data[self.informative_input_variables]
        data = data.apply(lambda column: column.apply(str) if self.get_variable_type(column.name) == "Categorical" else column)
        self.transformed_data = data.transform(lambda column: transform_variable(self.model[column.name], column))
    ...
    def get_target_frequencies(self, variable):
        ...
        jtcolumns = {}
        for jt in self.target_treatment_pairs:
            j, t = split_jt(jt)
            jtcolumn = []
            for i, _ in enumerate(partition):
                jtcolumn.append(len(varcol[(self.treatment_col == t) & (self.target_col == j) & (transform_variable(partition, varcol) == i)]))
    ...
    def get_uplift(self, reftarget, reftreatment, variable):
        ...
        refprobs = probs[join_jt(reftarget, reftreatment)]
        return pd.DataFrame({
            treatment: probs[join_jt(reftarget, treatment)] - refprobs
            for treatment in self.treatment_modalities
        }, index=probs.index)
