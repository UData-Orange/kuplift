...
class _Node:
    ...
    def discretize_vars_and_get_attributes_splits_costs(self, candidate_attributes=None):
        """For this node loop on attributes and get the optimal split for each one.

        Parameters
        ----------
        candidate_attributes : list[str] | None
            If provided, only these attributes are evaluated for candidate splits.
            If None, all node attributes are evaluated.

        Returns
        -------
        dict
            Dictionary {attribute: split_cost}.
        """
        all_features = list(self.x.columns)

        # Optional filtering hook used by higher-level tree classes
        if candidate_attributes is None:
            features = all_features
        else:
            candidate_set = set(candidate_attributes)
            features = [att for att in all_features if att in candidate_set]
