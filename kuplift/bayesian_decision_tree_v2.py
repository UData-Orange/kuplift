...
class BayesianDecisionTree(_Tree):
    ...
    def _select_candidate_attributes_for_node(self, terminal_node):
        """
        Hook for subclasses to restrict candidate attributes per node.

        Returns
        -------
        list[str] | None
            - None: evaluate all available attributes
            - list: evaluate only attributes in the list
        """
        return None
    ...
    def fit(self, data, treatment_col, y_col):
        ...
        while True:
            ...
            for terminal_node in self.terminal_nodes:
                candidate_attributes = self._select_candidate_attributes_for_node(terminal_node)

                # Always recompute candidate splits in this path because
                # candidate attribute subsets may vary at each iteration/node.
                ...
                # Once costs are updated, get the key of the minimal value split
